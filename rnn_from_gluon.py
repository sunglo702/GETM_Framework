...
# rnn_layer 作为RNN实例，在前向计算后回返回输出和隐藏状态，其中输出指的是隐藏层在各个时间步上计算并输出的隐藏状态，他们通常作为后续输层的输入。
num_steps = 35
...
train_and_predict_rnn_gluon(model, num_hiddens, vocab_size, ctx, corpus_indices, idx_to_char, char_to_idx, num_epochs,
